...
public class FuncionarioRepository {
    ...
    public ArrayList<Funcionario> getFuncionarios(String nomePesquisa) {
        ...
        try {
            ...
            if (nomePesquisa == "") {
                rs = st.executeQuery(
                        "SELECT * FROM tbfuncs AS F INNER JOIN tbcargos AS C ON F.cod_cargo = C.cod_cargo");
            } else {
                rs = st.executeQuery(
                        "SELECT * FROM tbfuncs AS F INNER JOIN tbcargos AS C ON F.cod_cargo = C.cod_cargo WHERE F.nome_func LIKE '%"
                                + nomePesquisa + "%'");
            }
            while (rs.next()) {
